 ...
 *
 *   Request -> authenticateToken -> req.user.id -> requireProjectAccess -> req.project -> handler
 *
 * Reads `:projectId` from the route params, verifies the authenticated
 * caller (req.user.id, set by whichever auth middleware runs immediately
 ...
 * req.user.id, req.params.projectId, and ownership.service.js -- so once
...
async function requireProjectAccess(req, res, next) {
  try {
    ...
    const ownership = await checkProjectOwnership(projectId, req.user.id);
    if (!ownership.exists) {
